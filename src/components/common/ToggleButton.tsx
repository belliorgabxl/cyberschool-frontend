...
export default function ToggleButton({ onToggle }: Props) {
  const [selected, setSelected] = useState<"motion-graphic" | "short-video">("motion-graphic");
  ...
  return (
    <div className="relative inline-flex border border-gray-200 rounded-lg overflow-hidden text-sm font-medium shadow-sm bg-gray-300">
      <div
        className="absolute inset-y-0 w-1/2 bg-white rounded-lg transition-transform duration-300"
        style={{
          transform:
            selected === "motion-graphic" ? "translateX(0%)" : "translateX(100%)",
        }}
      />
      ...
      <div className="relative z-10 flex">
        <button
          ...
          className={`flex items-center gap-2 px-16 py-2 transition-colors duration-300 ${
